			...
			if(bundleList.size() != 0) {
				...
				if(bundleList.size() == 1) {
					...
					if(vo.getOsType() == 4 && bundleList.get(0).getOsType()  == 4) {
						// app_resgist.html에서 요청한 ostype이 4이고
						// DB에서 받은 bundle에 등록되어있는 ostype도 4이면 등록 불가능
						return 1;
					}else if( vo.getOsType() != 4  && bundleList.get(0).getOsType() != 4){
						// app_resgist.html에서 요청한 ostype이 4가 아니고 ( iOS )
						// DB에서 받은 bundle에 등록되어있는 ostype도 4가아니면 등록불가능 ( iOS )
						return 1;
					}
					// 그외 서로 다른 OSTYPE이면 등록 가능
					else return 0;
				}
